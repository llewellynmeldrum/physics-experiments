 ...
 * shapes and use a user-provided texture with SetShapesTexture(), the purpose
 ...
 * raylib text and shapes can be drawn with a single draw call and it also
 * allows users to configure it the same way with their own fonts
 *
 *   CONFIGURATION:
...
void DrawLineBezier(Vector2 startPos, Vector2 endPos, float thick,
    ...
    for (int i = 1; i <= SPLINE_SEGMENT_DIVISIONS; i++) {
        ...
        current.x = previous.x +
                    (endPos.x - startPos.x) / (float)SPLINE_SEGMENT_DIVISIONS;
...
void DrawLineDashed(Vector2 startPos, Vector2 endPos, int dashSize,
    ...
    // If the line is too short for dashing or dash size is invalid, draw a
    // solid thick line
    ...
    while (distanceTraveled < lineLength) {
        ...
        // Update the distance traveled and move the current position for the
        // next dash
...
void DrawTriangleGradient(Vector2 v1, Vector2 v2, Vector2 v3, Color c1,
                          Color c2, Color c3) {
#if SUPPORT_QUADS_DRAW_MODE
    rlSetTexture(GetShapesTexture().id);
    Rectangle shapeRect = GetShapesTextureRectangle();
...
// NOTE: Vertex must be provided in counter-clockwise order
void DrawTriangleLinesEx(Vector2 v1, Vector2 v2, Vector2 v3, float thick,
                         Color color) {
    /*
    A sketch to make things simpler

    The exterior points are v1-3, the interior points are v4-6, and the exterior
    edges are e1-3
    ...
    */
    ...
    // The incenter of a triangle is equidistant from each edge, which is useful
    // for drawing a nice looking outline
    Vector2 incenter = {
        (e1Length * v1.x + e2Length * v2.x + e3Length * v3.x) / perimeter,
        (e1Length * v1.y + e2Length * v2.y + e3Length * v3.y) / perimeter};

    // The inradius of a triangle is the radius of the biggest circle that can
    // fit inside of said triangle That circle is also centered on the incenter
    float inradius =
        sqrtf(((semiperimeter - e1Length) * (semiperimeter - e2Length) *
               (semiperimeter - e3Length)) /
              semiperimeter);
    ...
    // In order for the scaling to be correct, the incenter has to be at the
    // origin (0, 0) when scaling
    Vector2 v4 = {incenter.x + (v1.x - incenter.x) * scale,
                  incenter.y + (v1.y - incenter.y) * scale};
    Vector2 v5 = {incenter.x + (v2.x - incenter.x) * scale,
                  incenter.y + (v2.y - incenter.y) * scale};
    Vector2 v6 = {incenter.x + (v3.x - incenter.x) * scale,
                  incenter.y + (v3.y - incenter.y) * scale};
...
void DrawRectanglePro(Rectangle rec, Vector2 origin, float rotation,
    ...
    if (rotation == 0.0f) {
        ...
        bottomRight.x = x + (dx + rec.width) * cosRotation -
                        (dy + rec.height) * sinRotation;
        bottomRight.y = y + (dx + rec.width) * sinRotation +
                        (dy + rec.height) * cosRotation;
    }
...
// Draw a gradient-filled rectangle with custom vertex colors, counter-clockwise
// color order
...
void DrawRectangleLinesEx(Rectangle rec, float thick, Color color) {
    ...
    if (thick > 0.0f) {
        ...
        Rectangle bottom = {rec.x, rec.y - thick + rec.height, rec.width,
                            thick};
        Rectangle left = {rec.x, rec.y + thick, thick,
                          rec.height - thick * 2.0f};
        Rectangle right = {rec.x - thick + rec.width, rec.y + thick, thick,
                           rec.height - thick * 2.0f};
        ...
        Rectangle top = {rec.x - thick, rec.y - thick, rec.width + thick * 2.0f,
                         thick};
        Rectangle bottom = {rec.x - thick, rec.y + rec.height,
                            rec.width + thick * 2.0f, thick};
...
void DrawRectangleRounded(Rectangle rec, float roundness, int segments,
    ...
    */
    ...
    rlBegin(RL_QUADS);
    // Draw all the 4 corners: [1] Upper Left Corner, [3] Upper Right Corner,
...
void DrawRectangleRoundedLinesEx(Rectangle rec, float roundness, int segments,
    ...
    if (thick >= 0.0f) {
        // Calculate corner radius
        radius = (rec.width > rec.height) ? (rec.height * roundness) / 2
                                          : (rec.width * roundness) / 2;
        if (radius <= 0.0f)
            return;
        ...
        // The maximum thickness the outline can have and still be rounded on
        // the interior edge is equal to the corner radius Put another way, when
        // `innerRadius <= 0`, the interior of the outline is just a normal
        ...
        if (innerRadius <= 0.0f) {
            ...
            DrawRectangleLinesEx((Rectangle){rec.x + outerRadius,
                                             rec.y + outerRadius,
                                             rec.width - outerRadius * 2.0f,
                                             rec.height - outerRadius * 2.0f},
                                 thick - outerRadius, color);
        } else {
        ...
        // Calculate number of segments to use for the corners
        if (segments < 4) {
            // Calculate the maximum angle between segments based on the error
            // rate (usually 0.5f)
            float th = acosf(
                2 * powf(1 - SMOOTH_CIRCLE_ERROR_RATE / outerRadius, 2) - 1);
            segments = (int)ceilf((2 * PI / th) / 4.0f);
            if (segments <= 0)
                segments = 4;
        }
        ...
        radius = (rec.width > rec.height) ? (rec.height * roundness) / 2
                                          : (rec.width * roundness) / 2;
        if (radius <= 0.0f)
            return; // Only possible if the rectangle has 0 width or height
        ...
        if (segments < 4) {
            ...
            // rate (usually 0.5f)
            float th = acosf(
                2 * powf(1 - SMOOTH_CIRCLE_ERROR_RATE / innerRadius, 2) - 1);
            segments = (int)ceilf((2 * PI / th) / 4.0f);
            if (segments <= 0)
                segments = 4;
        }
...
void DrawPolyLinesEx(Vector2 center, int sides, float radius, float rotation,
    ...
    for (int i = 0; i < sides; i++) {
        ...
        rlTexCoord2f(shapeRect.x / texShapes.width,
                     (shapeRect.y + shapeRect.height) / texShapes.height);
        rlVertex2f(center.x + cosf(centralAngle) * outerRadius,
                   center.y + sinf(centralAngle) * outerRadius);
        ...
        rlTexCoord2f((shapeRect.x + shapeRect.width) / texShapes.width,
                     shapeRect.y / texShapes.height);
        rlVertex2f(center.x + cosf(nextAngle) * outerRadius,
...
void DrawCircleGradient(Vector2 center, float radius, Color inner,
                        Color outer) {
    rlBegin(RL_TRIANGLES);
...
void DrawCircleSector(Vector2 center, float radius, float startAngle,
    ...
    // Drawing a whole circle, things get weird without limiting the circle to
    // 360 degrees
...
void DrawCircleSectorLinesEx(Vector2 center, float radius, float startAngle,
                             float endAngle, int segments, float thick,
                             Color color) {
    if (startAngle == endAngle)
        return;
    if (radius <= 0.0f)
    ...
    NOTE: Some considerations are different when `thick` is negative
          The vertices used here are still relevant, but would instead be
    outside of the circle S0 is always the center

    The circle sector outline is drawn in 3 main pieces, the circle outline, cap
    1, and cap 2 The circle outline is self explanatory Cap 1 covers the
    `startAngle` edge and cap 2 covers the `endAngle` edge S0 is the first
    shared point between the caps, and also the circle's center S1 is the second
    shared point (sometimes not shared) between the caps S1 is also C0 and C3 in
    this sketch. In certain cases, S1 goes outside of the circle and C0 and C3
    become different points C1 is one of cap 1's vertices that is on the inside
    edge of the circle outline C2 is like C1, but is also on the `startAngle`
    edge C4 is cap 2's vertex that corresponds with C1 C5 is cap 2's vertex that
    corresponds with C2, except on the `endAngle` edge
    ...
    */
    ...
    // So, we need to adjust the outline thickness of the "circle" for it to
    // look correct with fewer segments
    float apothem = radius * cosf(DEG2RAD * ((endAngle - startAngle) / 2.0f) /
                                  (float)segments);
    ...
    // The number of angle steps that come before C1 (from `startAngle`, counter
    // clockwise)
    int stepsBeforeC1 = 0;
    bool s1OutsideOfCircle = false;
    // The number of angle steps that come before C0 (from `startAngle`, counter
    // clockwise) Only used if S1 is outside of the circle
    int stepsBeforeC0 = 0;

    if (showCapLines) {
        if (thick >= 0.0f) {
            c2 = (Vector2){center.x + cosf(DEG2RAD * startAngle) * innerRadius,
                           center.y + sinf(DEG2RAD * startAngle) * innerRadius};
            c5 = (Vector2){center.x + cosf(DEG2RAD * endAngle) * innerRadius,
                           center.y + sinf(DEG2RAD * endAngle) * innerRadius};

            // For C1 and C4, we need to find the point that lies on the circle
            // (n-sided polygon, actually) We want C1 and C4 to be `thick`
            // pixels perpendicularly from the `startAngle` and `endAngle` edges
            // and to be on the `innerRadius` edge

            float c1Angle = RAD2DEG * asinf(thick / innerRadius);

            // There are more segments before C1 than there are segments being
            // drawn, so the whole circle sector must be covered
            ...
            // Do this after the previous check just in case `stepLength` is
            // really small and dividing by it produces a very large number
            stepsBeforeC1 = (int)(c1Angle / stepLength);

            // The angles of the vertices on the circle outline before and after
            // C1
            float vertexAngleBeforeC1 = stepLength * (float)stepsBeforeC1;
            float vertexAngleAfterC1 =
                stepLength * ((float)(stepsBeforeC1 + 1));

            /*
            Here is another sketch

            We know which outline line segment C1 is on (`vertexAngleBeforeC1`
            and `vertexAngleAfterC1`) Now we just need to know where on that
            line segment C1 is

            We can change our frame of reference so that `startAngle` is 0
            degrees and `center` is at the origin (0, 0) This makes the math
            much simpler because now we can just go straight down by `thick`
            pixels and use the horizontal line that passes through that point to
            determine where C1 is on our line segment

            The line segment is defined by p1 and p2, we need C1, which is on
            that edge The 'y' axis of C1 is equal to `thick` (within this
            modified frame of reference)
            ...
            */
            ...
            Vector2 p1 = {cosf(DEG2RAD * vertexAngleBeforeC1) * innerRadius,
                          sinf(DEG2RAD * vertexAngleBeforeC1) * innerRadius};
            Vector2 p2 = {cosf(DEG2RAD * vertexAngleAfterC1) * innerRadius,
                          sinf(DEG2RAD * vertexAngleAfterC1) * innerRadius};

            // Find the `t` of C1 between p1 and p2 ('t' as in `Lerp(start, end,
            // t)`) This is used to lerp between the actual vertices (outside of
            // our modified frame of reference) before and after C1
            float t = (p1.y - thick) / (p1.y - p2.y);

            Vector2 vertexBeforeCap1Vertex = {
                center.x + cosf(DEG2RAD * (startAngle + vertexAngleBeforeC1)) *
                               innerRadius,
                center.y + sinf(DEG2RAD * (startAngle + vertexAngleBeforeC1)) *
                               innerRadius};
            Vector2 vertexAfterCap1Vertex = {
                center.x + cosf(DEG2RAD * (startAngle + vertexAngleAfterC1)) *
                               innerRadius,
                center.y + sinf(DEG2RAD * (startAngle + vertexAngleAfterC1)) *
                               innerRadius};

            c1.x = vertexBeforeCap1Vertex.x +
                   (vertexAfterCap1Vertex.x - vertexBeforeCap1Vertex.x) * t;
            c1.y = vertexBeforeCap1Vertex.y +
                   (vertexAfterCap1Vertex.y - vertexBeforeCap1Vertex.y) * t;

            Vector2 vertexBeforeCap2Vertex = {
                center.x + cosf(DEG2RAD * (endAngle - vertexAngleBeforeC1)) *
                               innerRadius,
                center.y + sinf(DEG2RAD * (endAngle - vertexAngleBeforeC1)) *
                               innerRadius};
            Vector2 vertexAfterCap2Vertex = {
                center.x + cosf(DEG2RAD * (endAngle - vertexAngleAfterC1)) *
                               innerRadius,
                center.y + sinf(DEG2RAD * (endAngle - vertexAngleAfterC1)) *
                               innerRadius};

            c4.x = vertexBeforeCap2Vertex.x +
                   (vertexAfterCap2Vertex.x - vertexBeforeCap2Vertex.x) * t;
            c4.y = vertexBeforeCap2Vertex.y +
                   (vertexAfterCap2Vertex.y - vertexBeforeCap2Vertex.y) * t;
            ...
            `innerAngleBetweenCapEnds` is the angle of the diagonal line ('//')
            between the cap ends `S1Length` is the length of that line
            ...
            */
            ...
            float innerAngleBetweenCapEnds =
                ((endAngle - 90.0f) - (startAngle + 90.0f)) / 2.0f;
            float s1Length = thick / cosf(DEG2RAD * innerAngleBetweenCapEnds);

            // As `startAngle` and `endAngle` draw more of a circle, S1 goes
            // further out from the center It can go so far that it is outside
            // of the circle, by a lot This case needs to be detected and
            // handled If S1 is within the circle, nothing special needs to
            // happen But, if S1 is outside of the circle, we need to find the
            // two points (C0 and C3) where the line segments C0->C1 and C0->C3
            // intersect the circle outline, using the same method we used to
            // find C1 and C4
            if ((innerAngleBetweenCapEnds < 90.0f) &&
                (s1Length <= innerRadius)) {
                // S1 is inside of the circle

                float betweenStartAndEndAngle = (endAngle + startAngle) / 2.0f;
                c0 = (Vector2){
                    center.x +
                        cosf(DEG2RAD * betweenStartAndEndAngle) * s1Length,
                    center.y +
                        sinf(DEG2RAD * betweenStartAndEndAngle) * s1Length};
                c3 = c0;
                ...
                // If C1 and C4 are on the wrong side of S1, the whole circle
                // sector is covered
                if (c1ToS1Angle < 0.0f) {
                ...
                stepsBeforeC0 =
                    (int)((180.0f + RAD2DEG * asinf(thick / -innerRadius)) /
                          stepLength);

                // Reuse the code for finding C1 and C4 to find C0 and C3

                float vertexAngleBeforeC0 = stepLength * (float)stepsBeforeC0;
                float vertexAngleAfterC0 =
                    stepLength * ((float)(stepsBeforeC0 + 1));

                p1 = (Vector2){
                    cosf(DEG2RAD * vertexAngleBeforeC0) * innerRadius,
                    sinf(DEG2RAD * vertexAngleBeforeC0) * innerRadius};
                p2 =
                    (Vector2){cosf(DEG2RAD * vertexAngleAfterC0) * innerRadius,
                              sinf(DEG2RAD * vertexAngleAfterC0) * innerRadius};

                t = (p1.y - thick) / (p1.y - p2.y);

                vertexBeforeCap1Vertex = (Vector2){
                    center.x +
                        cosf(DEG2RAD * (startAngle + vertexAngleBeforeC0)) *
                            innerRadius,
                    center.y +
                        sinf(DEG2RAD * (startAngle + vertexAngleBeforeC0)) *
                            innerRadius};
                vertexAfterCap1Vertex = (Vector2){
                    center.x +
                        cosf(DEG2RAD * (startAngle + vertexAngleAfterC0)) *
                            innerRadius,
                    center.y +
                        sinf(DEG2RAD * (startAngle + vertexAngleAfterC0)) *
                            innerRadius};

                c0.x = vertexBeforeCap1Vertex.x +
                       (vertexAfterCap1Vertex.x - vertexBeforeCap1Vertex.x) * t;
                c0.y = vertexBeforeCap1Vertex.y +
                       (vertexAfterCap1Vertex.y - vertexBeforeCap1Vertex.y) * t;

                vertexBeforeCap2Vertex = (Vector2){
                    center.x +
                        cosf(DEG2RAD * (endAngle - vertexAngleBeforeC0)) *
                            innerRadius,
                    center.y +
                        sinf(DEG2RAD * (endAngle - vertexAngleBeforeC0)) *
                            innerRadius};
                vertexAfterCap2Vertex = (Vector2){
                    center.x + cosf(DEG2RAD * (endAngle - vertexAngleAfterC0)) *
                                   innerRadius,
                    center.y + sinf(DEG2RAD * (endAngle - vertexAngleAfterC0)) *
                                   innerRadius};

                c3.x = vertexBeforeCap2Vertex.x +
                       (vertexAfterCap2Vertex.x - vertexBeforeCap2Vertex.x) * t;
                c3.y = vertexBeforeCap2Vertex.y +
                       (vertexAfterCap2Vertex.y - vertexBeforeCap2Vertex.y) * t;
            }
        } else {
            float outerAngleBetweenCapEnds =
                ((endAngle + 90.0f) - (startAngle - 90.0f)) / 2.0f;
            float s1Length = thick / cosf(DEG2RAD * outerAngleBetweenCapEnds);
            float betweenStartAndEndAngle =
                180.0f + (endAngle + startAngle) / 2.0f;
            c0 = (Vector2){
                center.x + cosf(DEG2RAD * betweenStartAndEndAngle) * s1Length,
                center.y + sinf(DEG2RAD * betweenStartAndEndAngle) * s1Length};
            c3 = c0;

            c2 = (Vector2){center.x + cosf(DEG2RAD * startAngle) * outerRadius,
                           center.y + sinf(DEG2RAD * startAngle) * outerRadius};
            c5 = (Vector2){center.x + cosf(DEG2RAD * endAngle) * outerRadius,
                           center.y + sinf(DEG2RAD * endAngle) * outerRadius};

            // Change the frame of reference so that `center` is the origin and
            // `startAngle` is 0 degrees

            Vector2 c0Translated = {c0.x - center.x, c0.y - center.y};
            Vector2 circleVertex1 = {c2.x - center.x, c2.y - center.y};
            Vector2 circleVertex2 = {
                cosf(DEG2RAD * (startAngle + stepLength)) * outerRadius,
                sinf(DEG2RAD * (startAngle + stepLength)) * outerRadius};

            // Copied from "raymath.h" Vector2Rotate()
            float tempX = c0Translated.x;
            c0Translated.x = cosf(-DEG2RAD * startAngle) * tempX -
                             sinf(-DEG2RAD * startAngle) * c0Translated.y;
            c0Translated.y = sinf(-DEG2RAD * startAngle) * tempX +
                             cosf(-DEG2RAD * startAngle) * c0Translated.y;

            tempX = circleVertex1.x;
            circleVertex1.x = cosf(-DEG2RAD * startAngle) * tempX -
                              sinf(-DEG2RAD * startAngle) * circleVertex1.y;
            circleVertex1.y = sinf(-DEG2RAD * startAngle) * tempX +
                              cosf(-DEG2RAD * startAngle) * circleVertex1.y;

            tempX = circleVertex2.x;
            circleVertex2.x = cosf(-DEG2RAD * startAngle) * tempX -
                              sinf(-DEG2RAD * startAngle) * circleVertex2.y;
            circleVertex2.y = sinf(-DEG2RAD * startAngle) * tempX +
                              cosf(-DEG2RAD * startAngle) * circleVertex2.y;

            // Figure out the line that `circleVertex1` and `circleVertex2` are
            // on
            float rise = circleVertex1.y - circleVertex2.y;
            float run = circleVertex1.x - circleVertex2.x;
            // Get where that line intersects the horizontal line that
            // `c0Translated` is on
            ...
            c1 =
                (Vector2){c0.x + cosf(DEG2RAD * startAngle) * c1DistanceFromC0,
                          c0.y + sinf(DEG2RAD * startAngle) * c1DistanceFromC0};
            c4 = (Vector2){c0.x + cosf(DEG2RAD * endAngle) * c1DistanceFromC0,
                           c0.y + sinf(DEG2RAD * endAngle) * c1DistanceFromC0};

            if (c1DistanceFromC0 < 0.0f) {
                // The caps are intersecting each other

                Vector2 circleVertex3 = {c5.x - center.x, c5.y - center.y};
                Vector2 circleVertex4 = {
                    cosf(DEG2RAD * (endAngle - stepLength)) * outerRadius,
                    sinf(DEG2RAD * (endAngle - stepLength)) * outerRadius};

                tempX = circleVertex3.x;
                circleVertex3.x = cosf(-DEG2RAD * startAngle) * tempX -
                                  sinf(-DEG2RAD * startAngle) * circleVertex3.y;
                circleVertex3.y = sinf(-DEG2RAD * startAngle) * tempX +
                                  cosf(-DEG2RAD * startAngle) * circleVertex3.y;

                tempX = circleVertex4.x;
                circleVertex4.x = cosf(-DEG2RAD * startAngle) * tempX -
                                  sinf(-DEG2RAD * startAngle) * circleVertex4.y;
                circleVertex4.y = sinf(-DEG2RAD * startAngle) * tempX +
                                  cosf(-DEG2RAD * startAngle) * circleVertex4.y;

                // `startAngle` is 0 degrees within this frame of reference,
                // so C1 just goes horizontally out from C0
                Vector2 c1Translated = {c0Translated.x + c1DistanceFromC0,
                                        c0Translated.y};
                ...
                // Make the line between `circleVertex1` and `circleVertex2` a
                // horizontal line
                ...
                tempX = circleVertex1.x;
                circleVertex1.x =
                    cosf(-theta) * tempX - sinf(-theta) * circleVertex1.y;
                circleVertex1.y =
                    sinf(-theta) * tempX + cosf(-theta) * circleVertex1.y;

                tempX = circleVertex3.x;
                circleVertex3.x =
                    cosf(-theta) * tempX - sinf(-theta) * circleVertex3.y;
                circleVertex3.y =
                    sinf(-theta) * tempX + cosf(-theta) * circleVertex3.y;

                tempX = circleVertex4.x;
                circleVertex4.x =
                    cosf(-theta) * tempX - sinf(-theta) * circleVertex4.y;
                circleVertex4.y =
                    sinf(-theta) * tempX + cosf(-theta) * circleVertex4.y;

                tempX = c1Translated.x;
                c1Translated.x =
                    cosf(-theta) * tempX - sinf(-theta) * c1Translated.y;
                c1Translated.y =
                    sinf(-theta) * tempX + cosf(-theta) * c1Translated.y;

                // Find where the line that `circleVertex3` and `circleVertex4`
                // are on would intersect the line segment defined by
                // `circleVertex1` and `c1Translated`
                ...
                float t = (c1Translated.x - targetX) /
                          (c1Translated.x - circleVertex1.x);

                c1 = (Vector2){c1.x + (c2.x - c1.x) * t,
                               c1.y + (c2.y - c1.y) * t};
                c4 = c1;
                c0 = c1;
                c3 = c1;
    ...
    if (showCapLines) {
        ...
        if (thick >= 0.0f) {
            // Fill in the gaps between cap 1 and the circle outline and cap 2
            ...
            if (stepsBeforeC1 > 0) {
                ...
                for (int i = 0; i < stepsBeforeC1 / 2; i++) {
                    ...
                    rlVertex2f(
                        center.x +
                            cosf(DEG2RAD *
                                 (startAngle + (angle + stepLength * 2.0f))) *
                                innerRadius,
                        center.y +
                            sinf(DEG2RAD *
                                 (startAngle + (angle + stepLength * 2.0f))) *
                    ...
                    rlTexCoord2f((shapeRect.x + shapeRect.width) /
                                     texShapes.width,
                                 shapeRect.y / texShapes.height);
                    rlVertex2f(
                        center.x + cosf(DEG2RAD *
                                        (startAngle + (angle + stepLength))) *
                                       innerRadius,
                        center.y + sinf(DEG2RAD *
                                        (startAngle + (angle + stepLength))) *
                                       innerRadius);
                    ...
                    rlTexCoord2f(shapeRect.x / texShapes.width,
                                 shapeRect.y / texShapes.height);
                    rlVertex2f(center.x + cosf(DEG2RAD * (endAngle - angle)) *
                                              innerRadius,
                               center.y + sinf(DEG2RAD * (endAngle - angle)) *
                                              innerRadius);
                    ...
                    rlVertex2f(
                        center.x +
                            cosf(DEG2RAD * (endAngle - (angle + stepLength))) *
                                innerRadius,
                        center.y +
                            sinf(DEG2RAD * (endAngle - (angle + stepLength))) *
                                innerRadius);
                    ...
                    rlVertex2f(
                        center.x +
                            cosf(DEG2RAD *
                                 (endAngle - (angle + stepLength * 2.0f))) *
                                innerRadius,
                        center.y +
                            sinf(DEG2RAD *
                                 (endAngle - (angle + stepLength * 2.0f))) *
            ...
            if (s1OutsideOfCircle) {
                ...
                // There's a gap to fill
                    ...
                    for (int i = 0; i < verticesBetweenC0andC3 / 2; i++) {
                        ...
                        rlVertex2f(
                            center.x +
                                cosf(DEG2RAD * (angle + stepLength * 2.0f)) *
                                    innerRadius,
                            center.y +
                                sinf(DEG2RAD * (angle + stepLength * 2.0f)) *
                                    innerRadius);
                        ...
                        rlTexCoord2f((shapeRect.x + shapeRect.width) /
                                     ...
                                     shapeRect.y / texShapes.height);
                        rlVertex2f(
                            center.x + cosf(DEG2RAD * (angle + stepLength)) *
                                           innerRadius,
                            center.y + sinf(DEG2RAD * (angle + stepLength)) *
                                           innerRadius);
...
void DrawEllipseLines(int centerX, int centerY, float radiusH, float radiusV,
                      Color color) {
    DrawEllipseLinesV((Vector2){(float)centerX, (float)centerY}, radiusH,
                      radiusV, color);
}
...
void DrawEllipseLinesEx(Vector2 center, float radiusH, float radiusV,
                        float thick, Color color) {
    ...
    if (thick >= 0.0f) {
        ...
        // The outline is growing outside of the ellipse, so swap the inner and
        // outer radius
    ...
    for (int i = 0; i < 360; i += 10) {
        rlTexCoord2f(shapeRect.x / texShapes.width,
                     shapeRect.y / texShapes.height);
        rlVertex2f(center.x + cosf(DEG2RAD * i) * innerRadiusH,
                   center.y + sinf(DEG2RAD * i) * innerRadiusV);

        rlTexCoord2f(shapeRect.x / texShapes.width,
                     (shapeRect.y + shapeRect.height) / texShapes.height);
        rlVertex2f(center.x + cosf(DEG2RAD * (i + 10)) * innerRadiusH,
                   center.y + sinf(DEG2RAD * (i + 10)) * innerRadiusV);

        rlTexCoord2f((shapeRect.x + shapeRect.width) / texShapes.width,
                     (shapeRect.y + shapeRect.height) / texShapes.height);
        rlVertex2f(center.x + cosf(DEG2RAD * (i + 10)) * outerRadiusH,
                   center.y + sinf(DEG2RAD * (i + 10)) * outerRadiusV);

        rlTexCoord2f((shapeRect.x + shapeRect.width) / texShapes.width,
                     shapeRect.y / texShapes.height);
        rlVertex2f(center.x + cosf(DEG2RAD * i) * outerRadiusH,
                   center.y + sinf(DEG2RAD * i) * outerRadiusV);
    }
    rlEnd();
...
void DrawRingLinesEx(Vector2 center, float innerRadius, float outerRadius,
                     float startAngle, float endAngle, int segments,
                     float thick, Color color) {
    if (startAngle == endAngle)
        return;
    ...
    float apothem =
        outerRadius *
        cosf(DEG2RAD * ((endAngle - startAngle) / 2.0f) / (float)segments);
    ...
    if (thick >= 0.0f) {
        ...
        if (innerRadius <= 0.0f) {
            DrawCircleSectorLinesEx(center, outerRadius, startAngle, endAngle,
                                    segments, thick, color);
            return;
        }
    ...
    if (showCapLines) {
        if (thick >= 0.0f) {
            // Get the angle of the arc that has `thick` length along the inner
            // and outer radii
            ...
            if (endAngle - startAngle < cap1InnerAngleEnd * 2.0f)
                innerAnglesCrossEachOther = true;

            stepsBeforeInner = (int)(cap1InnerAngleEnd / stepLength);
            stepsBeforeOuter = (int)(cap1OuterAngleEnd / stepLength);

            // We need to find where `cap1InnerAngleEnd` intersects the edge
            // defined by `beforeInnerVertex` and `afterInnerVertex`
            //
            ...
            Vector2 beforeInnerVertex = {
                cosf(DEG2RAD * ((float)stepsBeforeInner * stepLength -
                                cap1InnerAngleEnd)) *
                    outerInnerRadius,
                sinf(DEG2RAD * ((float)stepsBeforeInner * stepLength -
                                cap1InnerAngleEnd)) *
                    outerInnerRadius};
            Vector2 afterInnerVertex = {
                cosf(DEG2RAD * ((float)(stepsBeforeInner + 1) * stepLength -
                                cap1InnerAngleEnd)) *
                    outerInnerRadius,
                sinf(DEG2RAD * ((float)(stepsBeforeInner + 1) * stepLength -
                                cap1InnerAngleEnd)) *
                    outerInnerRadius};
            tInner = beforeInnerVertex.y /
                     (beforeInnerVertex.y - afterInnerVertex.y);

            // The same as above, but for the outer edge
            Vector2 beforeOuterVertex = {
                cosf(DEG2RAD * ((float)stepsBeforeOuter * stepLength -
                                cap1OuterAngleEnd)) *
                    innerOuterRadius,
                sinf(DEG2RAD * ((float)stepsBeforeOuter * stepLength -
                                cap1OuterAngleEnd)) *
                    innerOuterRadius};
            Vector2 afterOuterVertex = {
                cosf(DEG2RAD * ((float)(stepsBeforeOuter + 1) * stepLength -
                                cap1OuterAngleEnd)) *
                    innerOuterRadius,
                sinf(DEG2RAD * ((float)(stepsBeforeOuter + 1) * stepLength -
                                cap1OuterAngleEnd)) *
                    innerOuterRadius};
            tOuter = beforeOuterVertex.y /
                     (beforeOuterVertex.y - afterOuterVertex.y);
        } else {
            // "Cap 1" is the outline on `startAngle` and "Cap 2" is the outline
            // on `endAngle`
            ...
            Cap 2 is a mirror copy of cap 1, the inside and outside vertices
            switch sides

            We're using a frame of reference where `center` is (0, 0) and
            `startAngle` is 0 degrees

            I0 is `innerInnerRadius` distance from `center` at `starAngle`
            I1 is `innerInnerRadius` distance from `center` at `starAngle +
            stepLength` I2 goes out from I0 perpendicular to `startAngle` O0 is
            the same as I0, except using `outerOuterRadius` instead of
            `innerInnerRadius` O1 is the same as I1, except using
            `outerOuterRadius` instead of `innerInnerRadius` O2 is the same as
            I2, except goes out from O0
            ...
            Notice that cap 1 and 2's I2->O2 and O0->O2 edges can't intersect at
            the same time, and, if there's any intersection, I0->I2 is one of
            the edges
            */

            Vector2 cap1O0 = {outerOuterRadius, 0.0f};
            Vector2 cap1O1 = {cosf(DEG2RAD * stepLength) * outerOuterRadius,
                              sinf(DEG2RAD * stepLength) * outerOuterRadius};

            // Assuming a linear interpolation such as `value = Lerp(start, end,
            // t)` We can find O2 by getting its 't' between O1.y and O0.y
            // (which is always greater than 1) We can solve for `t` using `t =
            // (start - value)/(start - end)` Since we know `end = 0` we can
            // simplify it to `t = (start - value)/start`
            ...
            Vector2 cap2O0 = {
                cosf(DEG2RAD * (endAngle - startAngle)) * outerOuterRadius,
                sinf(DEG2RAD * (endAngle - startAngle)) * outerOuterRadius};
            Vector2 cap2O1 = {
                cosf(DEG2RAD * (endAngle - startAngle - stepLength)) *
                    outerOuterRadius,
                sinf(DEG2RAD * (endAngle - startAngle - stepLength)) *
                    outerOuterRadius};
            Vector2 cap2O2 = {cap2O1.x + (cap2O0.x - cap2O1.x) * tOuter,
                              cap2O1.y + (cap2O0.y - cap2O1.y) * tOuter};
            ...
            Vector2 cap2I2 = {
                cap2O2.x -
                    cosf(DEG2RAD * (endAngle - startAngle)) * capLongEdgeLength,
                cap2O2.y - sinf(DEG2RAD * (endAngle - startAngle)) *
                               capLongEdgeLength};
            ...
            if (cap2I2.y - cap2O2.y != 0.0f) {
                // Find where the long edge of cap 2 intersects the long edge of
                // cap 1
                tCapLongEdgeCross = (cap2I2.y - thick) / (cap2I2.y - cap2O2.y);
                if ((tCapLongEdgeCross >= 0.0f) && (tCapLongEdgeCross <= 1.0f))
                    capsIntersect = true;
            }

            // Rotate the frame of reference so that cap 1's I0->I2 edge is a
            // vertical line
            ...
            // Avoid division by zero
            if (cap2I0.x - cap2I2.x != 0.0f)
                tCrossInner = (cap2I0.x - cap1I2.x) / (cap2I0.x - cap2I2.x);
            // Make sure `tCrossInner` is 0 when it should be (mitigate
            // floating-point rounding woes)
            if (innerInnerRadius <= 0.0f)
                tCrossInner = 0.0f;

            // The 't' of the intersection between O0 and O2 (`Lerp(O0, O2, t)`)
            float tCrossOuter = -1.0f;
            // Avoid division by zero
            if (cap2O0.x - cap2O2.x != 0.0f)
                tCrossOuter = (cap2O0.x - cap1O2.x) / (cap2O0.x - cap2O2.x);

            // With our additional information, calculate the vertices we need
            // outside of our modified frame of reference

            cap1O0 = (Vector2){
                center.x + cosf(DEG2RAD * startAngle) * outerOuterRadius,
                center.y + sinf(DEG2RAD * startAngle) * outerOuterRadius};
            cap1O1 =
                (Vector2){center.x + cosf(DEG2RAD * (startAngle + stepLength)) *
                                         outerOuterRadius,
                          center.y + sinf(DEG2RAD * (startAngle + stepLength)) *
                                         outerOuterRadius};
            cap1O2 = (Vector2){cap1O1.x + (cap1O0.x - cap1O1.x) * tOuter,
                               cap1O1.y + (cap1O0.y - cap1O1.y) * tOuter};

            cap2O0 = (Vector2){
                center.x + cosf(DEG2RAD * endAngle) * outerOuterRadius,
                center.y + sinf(DEG2RAD * endAngle) * outerOuterRadius};
            cap2O1 =
                (Vector2){center.x + cosf(DEG2RAD * (endAngle - stepLength)) *
                                         outerOuterRadius,
                          center.y + sinf(DEG2RAD * (endAngle - stepLength)) *
                                         outerOuterRadius};
            cap2O2 = (Vector2){cap2O1.x + (cap2O0.x - cap2O1.x) * tOuter,
                               cap2O1.y + (cap2O0.y - cap2O1.y) * tOuter};

            // cap1I0 = (Vector2){ center.x +
            // cosf(DEG2RAD*startAngle)*innerInnerRadius, center.y +
            // sinf(DEG2RAD*startAngle)*innerInnerRadius };
            cap1I2 = (Vector2){
                cap1O2.x - cosf(DEG2RAD * startAngle) * capLongEdgeLength,
                cap1O2.y - sinf(DEG2RAD * startAngle) * capLongEdgeLength};

            cap2I0 = (Vector2){
                center.x + cosf(DEG2RAD * endAngle) * innerInnerRadius,
                center.y + sinf(DEG2RAD * endAngle) * innerInnerRadius};
            cap2I2 = (Vector2){
                cap2O2.x - cosf(DEG2RAD * endAngle) * capLongEdgeLength,
                cap2O2.y - sinf(DEG2RAD * endAngle) * capLongEdgeLength};

            if (capsIntersect) {
                capIntersectionVertex = (Vector2){
                    cap2I2.x + (cap2O2.x - cap2I2.x) * tCapLongEdgeCross,
                    cap2I2.y + (cap2O2.y - cap2I2.y) * tCapLongEdgeCross};

                cap2I2 =
                    (Vector2){cap2I0.x + (cap2I2.x - cap2I0.x) * tCrossInner,
                              cap2I0.y + (cap2I2.y - cap2I0.y) * tCrossInner};
                cap1I2 = cap2I2;
            } else if ((tCrossOuter >= 0.0f) && (tCrossOuter <= 1.0f)) {
                cap2O2 =
                    (Vector2){cap2O0.x + (cap2O2.x - cap2O0.x) * tCrossOuter,
                              cap2O0.y + (cap2O2.y - cap2O0.y) * tCrossOuter};
                cap1O2 = cap2O2;
    ...
    for (int i = 0; i < segments; i++) {
        ...
        rlTexCoord2f(shapeRect.x / texShapes.width,
                     (shapeRect.y + shapeRect.height) / texShapes.height);
        rlVertex2f(center.x + cosf(DEG2RAD * angle) * outerInnerRadius,
                   center.y + sinf(DEG2RAD * angle) * outerInnerRadius);

        rlTexCoord2f(shapeRect.x / texShapes.width,
                     shapeRect.y / texShapes.height);
        rlVertex2f(center.x + cosf(DEG2RAD * angle) * innerInnerRadius,
                   center.y + sinf(DEG2RAD * angle) * innerInnerRadius);

        rlTexCoord2f((shapeRect.x + shapeRect.width) / texShapes.width,
                     shapeRect.y / texShapes.height);
        rlVertex2f(
            center.x + cosf(DEG2RAD * (angle + stepLength)) * innerInnerRadius,
            center.y + sinf(DEG2RAD * (angle + stepLength)) * innerInnerRadius);

        rlTexCoord2f((shapeRect.x + shapeRect.width) / texShapes.width,
                     (shapeRect.y + shapeRect.height) / texShapes.height);
        rlVertex2f(
            center.x + cosf(DEG2RAD * (angle + stepLength)) * outerInnerRadius,
            center.y + sinf(DEG2RAD * (angle + stepLength)) * outerInnerRadius);

        // `outerRadius` outline
        rlTexCoord2f(shapeRect.x / texShapes.width,
                     (shapeRect.y + shapeRect.height) / texShapes.height);
        rlVertex2f(center.x + cosf(DEG2RAD * angle) * outerOuterRadius,
                   center.y + sinf(DEG2RAD * angle) * outerOuterRadius);

        rlTexCoord2f(shapeRect.x / texShapes.width,
                     shapeRect.y / texShapes.height);
        rlVertex2f(center.x + cosf(DEG2RAD * angle) * innerOuterRadius,
                   center.y + sinf(DEG2RAD * angle) * innerOuterRadius);

        rlTexCoord2f((shapeRect.x + shapeRect.width) / texShapes.width,
                     shapeRect.y / texShapes.height);
        rlVertex2f(
            center.x + cosf(DEG2RAD * (angle + stepLength)) * innerOuterRadius,
            center.y + sinf(DEG2RAD * (angle + stepLength)) * innerOuterRadius);

        rlTexCoord2f((shapeRect.x + shapeRect.width) / texShapes.width,
                     (shapeRect.y + shapeRect.height) / texShapes.height);
        rlVertex2f(
            center.x + cosf(DEG2RAD * (angle + stepLength)) * outerOuterRadius,
            center.y + sinf(DEG2RAD * (angle + stepLength)) * outerOuterRadius);
    ...
    if (showCapLines) {
        if (thick >= 0.0f) {
            ...
            for (int i = 0; i < stepsBeforeOuter; i++) {
                ...
                rlTexCoord2f(shapeRect.x / texShapes.width,
                             shapeRect.y / texShapes.height);
                rlVertex2f(center.x + cosf(DEG2RAD *
                                           (startAngle + angle + stepLength)) *
                                          outerInnerRadius,
                           center.y + sinf(DEG2RAD *
                                           (startAngle + angle + stepLength)) *
                ...
                rlTexCoord2f((shapeRect.x + shapeRect.width) / texShapes.width,
                             shapeRect.y / texShapes.height);
                rlVertex2f(
                    center.x + cosf(DEG2RAD * (endAngle - angle - stepLength)) *
                                   innerOuterRadius,
                    center.y + sinf(DEG2RAD * (endAngle - angle - stepLength)) *
                                   innerOuterRadius);
            ...
            // Cap 1
            Vector2 cap1OuterVertexBeforeEnd = {
                center.x +
                    cosf(DEG2RAD * (startAngle + angle)) * innerOuterRadius,
                center.y +
                    sinf(DEG2RAD * (startAngle + angle)) * innerOuterRadius};
            Vector2 cap1OuterVertexAfterEnd = {
                center.x + cosf(DEG2RAD * (startAngle + angle + stepLength)) *
                               innerOuterRadius,
                center.y + sinf(DEG2RAD * (startAngle + angle + stepLength)) *
                               innerOuterRadius};
            Vector2 cap1InnerVertexBeforeEnd = {
                center.x +
                    cosf(DEG2RAD * (startAngle + angle +
                                    (float)innerStepsLeft * stepLength)) *
                        outerInnerRadius,
                center.y +
                    sinf(DEG2RAD * (startAngle + angle +
                                    (float)innerStepsLeft * stepLength)) *
                        outerInnerRadius};
            Vector2 cap1InnerVertexAfterEnd = {
                center.x +
                    cosf(DEG2RAD * (startAngle + angle +
                                    (float)(innerStepsLeft + 1) * stepLength)) *
                        outerInnerRadius,
                center.y +
                    sinf(DEG2RAD * (startAngle + angle +
                                    (float)(innerStepsLeft + 1) * stepLength)) *
                        outerInnerRadius};
            Vector2 cap1InnerVertexEnd = {
                cap1InnerVertexBeforeEnd.x +
                    (cap1InnerVertexAfterEnd.x - cap1InnerVertexBeforeEnd.x) *
                        tInner,
                cap1InnerVertexBeforeEnd.y +
                    (cap1InnerVertexAfterEnd.y - cap1InnerVertexBeforeEnd.y) *
                        tInner};
            Vector2 cap1OuterVertexEnd = {
                cap1OuterVertexBeforeEnd.x +
                    (cap1OuterVertexAfterEnd.x - cap1OuterVertexBeforeEnd.x) *
                        tOuter,
                cap1OuterVertexBeforeEnd.y +
                    (cap1OuterVertexAfterEnd.y - cap1OuterVertexBeforeEnd.y) *
                        tOuter};

            // Cap 2
            Vector2 cap2OuterVertexBeforeEnd = {
                center.x +
                    cosf(DEG2RAD * (endAngle - angle)) * innerOuterRadius,
                center.y +
                    sinf(DEG2RAD * (endAngle - angle)) * innerOuterRadius};
            Vector2 cap2OuterVertexAfterEnd = {
                center.x + cosf(DEG2RAD * (endAngle - angle - stepLength)) *
                               innerOuterRadius,
                center.y + sinf(DEG2RAD * (endAngle - angle - stepLength)) *
                               innerOuterRadius};
            Vector2 cap2InnerVertexBeforeEnd = {
                center.x +
                    cosf(DEG2RAD * (endAngle - angle -
                                    (float)innerStepsLeft * stepLength)) *
                        outerInnerRadius,
                center.y +
                    sinf(DEG2RAD * (endAngle - angle -
                                    (float)innerStepsLeft * stepLength)) *
                        outerInnerRadius};
            Vector2 cap2InnerVertexAfterEnd = {
                center.x +
                    cosf(DEG2RAD * (endAngle - angle -
                                    (float)(innerStepsLeft + 1) * stepLength)) *
                        outerInnerRadius,
                center.y +
                    sinf(DEG2RAD * (endAngle - angle -
                                    (float)(innerStepsLeft + 1) * stepLength)) *
                        outerInnerRadius};
            Vector2 cap2InnerVertexEnd = {
                cap2InnerVertexBeforeEnd.x +
                    (cap2InnerVertexAfterEnd.x - cap2InnerVertexBeforeEnd.x) *
                        tInner,
                cap2InnerVertexBeforeEnd.y +
                    (cap2InnerVertexAfterEnd.y - cap2InnerVertexBeforeEnd.y) *
                        tInner};
            Vector2 cap2OuterVertexEnd = {
                cap2OuterVertexBeforeEnd.x +
                    (cap2OuterVertexAfterEnd.x - cap2OuterVertexBeforeEnd.x) *
                        tOuter,
                cap2OuterVertexBeforeEnd.y +
                    (cap2OuterVertexAfterEnd.y - cap2OuterVertexBeforeEnd.y) *
                        tOuter};

            int stepsCount = (innerAnglesCrossEachOther) ? totalStepsLeft / 2
                                                         : innerStepsLeft;
            ...
            for (int i = 0; i < stepsCount / 2; i++) {
                ...
                rlTexCoord2f(shapeRect.x / texShapes.width,
                             (shapeRect.y + shapeRect.height) /
                                 texShapes.height);
                rlVertex2f(cap1OuterVertexBeforeEnd.x,
                           cap1OuterVertexBeforeEnd.y);
                ...
                rlTexCoord2f(shapeRect.x / texShapes.width,
                             (shapeRect.y + shapeRect.height) /
                                 texShapes.height);
                rlVertex2f(cap2OuterVertexBeforeEnd.x,
                           cap2OuterVertexBeforeEnd.y);
                ...
                rlVertex2f(
                    center.x +
                        cosf(DEG2RAD * (endAngle - angle - stepLength * 2.0f)) *
                            outerInnerRadius,
                    center.y +
                        sinf(DEG2RAD * (endAngle - angle - stepLength * 2.0f)) *
                            outerInnerRadius);
            ...
            // When the inner angles coming from `startAngle` and `endAngle`
            // cross each other, the `*innerVertexEnd` vertices go past each
            // other and cause the geometry to intersect itself
            if (innerAnglesCrossEachOther) {
                // We need to find where the line defined by
                // `cap1InnerVertexEnd` and `cap1OuterVertexEnd` intersects the
                // line defined by `cap2InnerVertexEnd` and `cap2OuterVertexEnd`
                // That point is then used instead to prevent the outline from
                // intersecting itself

                // Make `cap1InnerVertexEnd` the origin and the angle to
                // `cap1OuterVertexEnd` 0 degrees
                Vector2 tempCap1OuterVertexEnd = {
                    cap1OuterVertexEnd.x - cap1InnerVertexEnd.x,
                    cap1OuterVertexEnd.y - cap1InnerVertexEnd.y};
                Vector2 tempCap2InnerVertexEnd = {
                    cap2InnerVertexEnd.x - cap1InnerVertexEnd.x,
                    cap2InnerVertexEnd.y - cap1InnerVertexEnd.y};
                Vector2 tempCap2OuterVertexEnd = {
                    cap2OuterVertexEnd.x - cap1InnerVertexEnd.x,
                    cap2OuterVertexEnd.y - cap1InnerVertexEnd.y};

                float rotateBy =
                    -atan2f(tempCap1OuterVertexEnd.y, tempCap1OuterVertexEnd.x);
                // We only need the y coordinates, so only rotate the y
                // coordinates
                float start = sinf(rotateBy) * tempCap2InnerVertexEnd.x +
                              cosf(rotateBy) * tempCap2InnerVertexEnd.y;
                float end = sinf(rotateBy) * tempCap2OuterVertexEnd.x +
                            cosf(rotateBy) * tempCap2OuterVertexEnd.y;
                float tCross = start / (start - end);

                Vector2 intersection = {
                    cap2InnerVertexEnd.x +
                        (cap2OuterVertexEnd.x - cap2InnerVertexEnd.x) * tCross,
                    cap2InnerVertexEnd.y +
                        (cap2OuterVertexEnd.y - cap2InnerVertexEnd.y) * tCross};

                if (segments % 2 == 0) {
                    // There are an even number of segments, so there's 1 vertex
                    // exactly in the middle

                    Vector2 middleInnerVertex = {
                    ...
                    // There are an odd number of segments, so there are 2
                    // vertices in the middle

                    Vector2 middleInnerVertex1 = {
                        center.x + cosf(DEG2RAD * (startAngle + angle)) *
                                       outerInnerRadius,
                        center.y + sinf(DEG2RAD * (startAngle + angle)) *
                                       outerInnerRadius};
                    Vector2 middleInnerVertex2 = {
                        center.x + cosf(DEG2RAD * (endAngle - angle)) *
                                       outerInnerRadius,
                        center.y + sinf(DEG2RAD * (endAngle - angle)) *
                                       outerInnerRadius};
                ...
                rlTexCoord2f(shapeRect.x / texShapes.width,
                             shapeRect.y / texShapes.height);
                rlVertex2f(cap1InnerVertexBeforeEnd.x,
                           cap1InnerVertexBeforeEnd.y);
                ...
                rlTexCoord2f((shapeRect.x + shapeRect.width) / texShapes.width,
                             (shapeRect.y + shapeRect.height) /
                                 texShapes.height);
                rlVertex2f(cap2InnerVertexBeforeEnd.x,
                           cap2InnerVertexBeforeEnd.y);
            }
        } else {
            // Cap 1
            Vector2 cap1FirstInnerVertex = {
                center.x + cosf(DEG2RAD * startAngle) * innerInnerRadius,
                center.y + sinf(DEG2RAD * startAngle) * innerInnerRadius};
            ...
            // Cap 2
            Vector2 cap2FirstInnerVertex = {
                center.x + cosf(DEG2RAD * endAngle) * innerInnerRadius,
                center.y + sinf(DEG2RAD * endAngle) * innerInnerRadius};
            Vector2 cap2FirstOuterVertex = {
                center.x + cosf(DEG2RAD * endAngle) * outerOuterRadius,
                center.y + sinf(DEG2RAD * endAngle) * outerOuterRadius};
...
void DrawSplineLinear(const Vector2 *points, int pointCount, float thick,
    ...
    for (int i = 0; i < pointCount - 1; i++) {
        delta = (Vector2){points[i + 1].x - points[i].x,
...
void DrawSplineBezierQuadratic(const Vector2 *points, int pointCount,
    ...
    if (pointCount >= 3) {
        for (int i = 0; i < pointCount - 2; i += 2)
            DrawSplineSegmentBezierQuadratic(points[i], points[i + 1],
                                             points[i + 2], thick, color);
...
void DrawSplineBezierCubic(const Vector2 *points, int pointCount, float thick,
    ...
    if (pointCount >= 4) {
        for (int i = 0; i < pointCount - 3; i += 3)
            DrawSplineSegmentBezierCubic(points[i], points[i + 1],
                                         points[i + 2], points[i + 3], thick,
                                         color);
...
void DrawSplineSegmentBezierQuadratic(Vector2 p1, Vector2 c2, Vector2 p3,
    ...
    for (int i = 1; i <= SPLINE_SEGMENT_DIVISIONS; i++) {
        ...
        // NOTE: The easing functions aren't suitable here because they don't
        // take a control point
...
Vector2 GetSplinePointBezierQuadratic(Vector2 startPos, Vector2 controlPos,
...
bool CheckCollisionCircles(Vector2 center1, float radius1, Vector2 center2,
    ...
    float distanceSquared =
        dx * dx + dy * dy; // Distance between centers squared
