...
struct DebugToggle {
    ...
    constexpr DebugToggle(const char *a_name, bool a_enabled,
                          KeyboardKey a_keybind)
        : name(a_name), default_status(a_enabled), keybind(a_keybind),
          keybind_name(magic_enum::enum_name(a_keybind).substr(4)) {}
    auto draw_label(glm::vec2 pos, bool enabled) const -> void {
        draw_label_px(pos,
                      std::format("[{:^3}] -> {} : {}", keybind_name, name,
                                  (enabled ? "on" : "off")),
                      (enabled ? colors::make_rgb(32, 128, 12)
                               : colors::make_rgb(16, 64, 6)));
    }
...
#define LIST_DEBUG_TOGGLES                                                     \
    ...
    X(render_paused, false, KEY_U)                                             \
    X(show_labels, false, KEY_L)                                               \
    X(draw_occupancy_grid_dbg, false, KEY_O)                                    \
    X(draw_quadtree_residency, false, KEY_NULL)                                    \
    X(show_hud, true, KEY_H)                                    \
    X(draw_cursor_pos, false, KEY_C)                                           \
    X(show_aabb, false, KEY_A)                                                 \
    X(show_velocity_vector, false, KEY_V)                                       \
    X(use_pair_dedup, false, KEY_J)
...
//         constexpr std::size_t operator()(std::pair<A,B> const& e) const
//         noexcept{
//             return std::hash<A>{}(e.first) ^ (std::hash<B>{}(e.second) <<
//             3U);
//         }
//     };
// };
