...
template<>
constexpr inline auto px_to_meters(glm::vec2 in){
    return in * metersPerPixel
    * glm::vec2{1.0f,-1.0f} // flip y axis
    - glm::vec2{screenExtentX*0.5f, -screenExtentY*0.5f};  // move to center 
}

template<>
constexpr inline auto meters_to_px(glm::vec2 in){ 
    return in * pixelsPerMeter
        * glm::vec2{1.0f,-1.0f}  // flip y axis
        + glm::vec2{screenExtentX_px*0.5f, +screenExtentY_px*0.5f};  // move to center 
}
...
inline auto to_color(glm::vec4 c){ return ColorFromNormalized(Vector4(c.r,c.g,c.b,c.a)); }
